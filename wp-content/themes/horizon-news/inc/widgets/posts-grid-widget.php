<?php
if ( ! class_exists( 'Horizon_News_Posts_Grid_Widget' ) ) {
	/**
	 * Adds Horizon_News_Posts_Grid_Widget Widget.
	 */
	class Horizon_News_Posts_Grid_Widget extends WP_Widget {

		/**
		 * Register widget with WordPress.
		 */
		public function __construct() {
			$horizon_news_grid_widget_ops = array(
				'classname'   => 'ascendoor-widget magazine-grid-section style-1',
				'description' => __( 'Retrive Posts Grid Widgets', 'horizon-news' ),
			);
			parent::__construct(
				'horizon_news_grid_widget',
				__( 'Ascendoor Posts Grid Widget', 'horizon-news' ),
				$horizon_news_grid_widget_ops
			);
		}
				...
				<div class="section-header">
					...
					if ( ! empty( $grid_button_label ) ) {
						...
						<a href="<?php echo esc_url( $grid_button_link ); ?>" class="mag-view-all-link">
							<span><?php echo esc_html( $grid_button_label ); ?></span>
						</a>
					<?php } ?>
				</div>
			<?php } ?>
				...
				<div class="magazine-grid-section-wrapper">
					...
					if ( $query->have_posts() ) :
						while ( $query->have_posts() ) :
							$query->the_post();
							?>
							<div class="mag-post-single <?php echo esc_attr( has_post_thumbnail() ? 'has-image' : '' ); ?>">
