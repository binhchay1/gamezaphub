	...
	class MTSContact {
		...
		public function init() {
			...
			if ( ! defined( 'DOING_AJAX' ) || ! DOING_AJAX ) {
				if ( ! empty( $_POST['action'] ) && 'mtscontact' === $_POST['action'] ) { // PHPCS:ignore
					if ( $this->validate() ) {
						if ( ! $this->send_mail() ) {
							$this->errors['sendmail'] = __( 'An error occurred. Please contact site administrator.', 'schema' );
						} else {
		...
		public function register_scripts() {
				...
				wp_localize_script( 'mtscontact', 'mtscontact', array(
		...
		private function validate() {
			...
			if ( ! empty( $_POST['mtscontact_captcha'] ) ) {  // PHPCS:ignore
					$this->errors['captcha'] = __( 'Please try again.', 'schema' );
			}

			// name field
			$name = trim( str_replace( array( '\n', '\r', '<', '>' ), '', strip_tags( $_POST['mtscontact_name'] ) ) ); // PHPCS:ignore
			if ( empty( $name ) ) {
				$this->errors['name'] = __( 'Please enter your name.', 'schema' );
			}

			// email field
			$useremail = trim( $_POST['mtscontact_email'] );  // PHPCS:ignore
			if ( ! is_email( $useremail ) ) {
				$this->errors['email'] = __( 'Please enter a valid email address.', 'schema' );
			}

			// message field
			$message = strip_tags( $_POST['mtscontact_message'] );  // PHPCS:ignore
			if ( empty( $message ) ) {
...
function mts_contact_form() {
	echo mts_get_contact_form();  // PHPCS:ignore
}
