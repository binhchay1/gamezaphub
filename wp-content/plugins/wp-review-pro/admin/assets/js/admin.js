...
( function( $, google ) {
	...
	function convertSchema() {
		var sourceEl, destEl, nonce, btn, doingEl, doneEl, countEl;

		btn      = $( '#wp-review-convert-schema' );
		sourceEl = $( '#wpr-convert-schema-source' );
		destEl   = $( '#wpr-convert-schema-dest' );
		nonce    = $( '#wp-review-convert-schema-nonce' ).val();
		doingEl  = $( '#wp-review-doing-convert-schema' );
		doneEl   = $( '#wp-review-done-convert-schema' );
		countEl  = $( '#wp-review-converted-count' );
		...
		function onSuccess( response ) {
			if ( response.done ) {
				...
				convert( response.source, response.dest, response.page );
			}
		}

		function onError( error ) {
			alert( wprImportVars.server_error );
			console.error( error );
		}

		function convert( source, dest, page ) {
			doingEl.show();
			btn.prop( 'disabled', true );
			doneEl.hide();

			var request = wp.ajax.post( 'wp_review_convert_schema', {
				source: source,
				dest: dest,
				page: page || 1,
				_ajax_nonce: nonce
		...
		$( document ).on( 'click', '#wp-review-convert-schema', function( ev ) {
			ev.preventDefault();

			convert( sourceEl.val(), destEl.val(), 0 );
		});
	}
