...
function wp_review_admin_style( $hook_suffix ) {
	if ( ! in_array( $hook_suffix, array( 'post-new.php', 'edit-comments.php', 'post.php', 'edit.php', 'widgets.php', 'settings_page_wp-review-pro' ), true ) ) {
		return;
	}
	...
	if ( in_array( $hook_suffix, array( 'edit.php', 'edit-comments.php', 'settings_page_wp-review-pro' ), true ) ) {
	...
	if ( in_array( $hook_suffix, array( 'settings_page_wp-review-pro', 'post.php', 'post-new.php' ), true ) ) {
	...
	if ( in_array( $hook_suffix, array( 'settings_page_wp-review-pro' ), true ) ) {
		wp_enqueue_media();
	...
	wp_localize_script(
		...
		array(
			...
			'importDemoDone'               => __( 'Importing process finished!', 'wp-review' ),
			'top'                          => __( 'Top', 'wp-review' ),
			'right'                        => __( 'Right', 'wp-review' ),
			'bottom'                       => __( 'Bottom', 'wp-review' ),
			'left'                         => __( 'Left', 'wp-review' ),
			'bgColor'                      => __( 'Background color', 'wp-review' ),
			'showAsBadge'                  => __( 'Show as badge', 'wp-review' ),
			'showAsBadgeOnly'              => __( 'Applies only if Show as Badge is enabled', 'wp-review' ),
			'reviewBadge'                  => __( 'Review badge', 'wp-review' ),
		)
	);

	if ( in_array( $hook_suffix, array( 'edit.php', 'edit-comments.php' ), true ) ) {
...
/**
 * Adds converted posts query.
 *
 * @param WP_Query $query Query object.
 */
function wp_review_add_converted_posts_query( $query ) {
	if ( isset( $_GET['converted'] ) ) {
		$converted_posts = get_option( 'wp_review_converted_posts', array() );
		if ( ! empty( $converted_posts ) ) {
			$query->query_vars['post_type']   = 'any';
			$query->query_vars['post_status'] = 'any';
			$query->query_vars['post__in']    = $converted_posts;
		}
	}
}
add_filter( 'parse_query', 'wp_review_add_converted_posts_query' );


/**
 * Clears external API cache when updating expired time options.
 *
 * @since 3.4.2
 *
 * @param array $old_value Old plugin options.
 * @param array $value     New plugin options.
 */
function wp_review_clear_external_api_cache( $old_value, $value ) {
	$names = array();

	if ( empty( $old_value['yelp_cache_expired_time'] ) || empty( $value['yelp_cache_expired_time'] ) || $old_value['yelp_cache_expired_time'] !== $value['yelp_cache_expired_time'] ) {
		$names[] = 'wp-review-yelp-business-reviews';
		$names[] = 'wp-review-yelp-business';
		$names[] = 'wp-review-yelp-search';
	}

	if ( empty( $old_value['google_cache_expired_time'] ) || empty( $value['google_cache_expired_time'] ) || $old_value['google_cache_expired_time'] !== $value['google_cache_expired_time'] ) {
		$names[] = 'wp-review-google-place-reviews';
	}

	if ( empty( $old_value['facebook_cache_expired_time'] ) || empty( $value['facebook_cache_expired_time'] ) || $old_value['facebook_cache_expired_time'] !== $value['facebook_cache_expired_time'] ) {
		$names[] = 'wp_review_facebook_page_';
		$names[] = 'wp-review-facebook-reviews';
	}

	if ( ! empty( $names ) ) {
		wp_review_clear_transient_by_contained_string( $names );
	}
}
add_action( 'update_option_wp_review_options', 'wp_review_clear_external_api_cache', 10, 2 );
