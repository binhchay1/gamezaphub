...
if ( wp_review_option( 'show_on_thumbnails' ) ) {
	...
	function wp_review_image_html( $html, $post_id, $post_image_id, $size, $attr ) {
		...
		if ( $image_sizes && is_array( $image_sizes ) && in_array( $size, $image_sizes, true ) ) {
			$args = array(
				'color'          => '#fff',
...
function wp_review_inject_data( $content ) {
	...
	if ( 'bottom' === $location || 'both' === $location ) {
		...
		if ( $multipage ) {
			if ( intval( $page ) === intval( $numpages ) ) {
				$content .= $review;
				return $content;
			}
			return $content;
...
function wp_review_show_total( $echo = true, $class = 'review-total-only', $post_id = null, $args = array() ) {
	...
	if ( $echo ) {
		echo $review; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	} else {
		return $review;
	}
}
...
function wp_review_has_reviewed( $post_id, $user_id, $ip = null, $type = 'any' ) {
	...
	if ( $user_id ) {
		return ! wp_review_option( 'multi_reviews_per_account' ) && wp_review_has_reviewed_by_user_id( $post_id, $user_id, $ip, $type );
	}
...
function wp_review_get_user_rating_setup( $post_id ) {
	$user_review_type = get_post_meta( $post_id, 'wp_review_userReview', true );
	if ( '' === $user_review_type ) { // New post.
		$user_review_type = wp_review_option( 'default_user_review_type', WP_REVIEW_REVIEW_DISABLED );
	}

	if ( is_array( $user_review_type ) ) {
		$user_review_type = $user_review_type[0]; // For the compatibility.
	}

	$user_review_type = (int) $user_review_type;

	if ( ! $user_review_type && wp_review_option( 'global_user_rating' ) && ! get_post_meta( $post_id, 'wp_review_override_global_user_rating', true ) ) {
		$user_review_type = WP_REVIEW_REVIEW_VISITOR_ONLY;
	}

	// For the compatibility.
	if ( 1 === $user_review_type ) {
		if ( get_post_meta( $post_id, 'wp_review_through_comment', true ) ) {
			$user_review_type = WP_REVIEW_REVIEW_ALLOW_BOTH;
		} else {
			$user_review_type = WP_REVIEW_REVIEW_VISITOR_ONLY;
		}
	}

	return $user_review_type;
}
...
function wp_review_get_post_review_type( $post_id = null ) {
	...
	if ( in_array( get_post_type( $post_id ), wp_review_get_excluded_post_types() ) ) {
		return '';
	}
	...
	if ( wp_review_option( 'global_user_rating', false ) && ! get_post_meta( $post_id, 'wp_review_override_global_user_rating', true ) && ( ! $type || 'none' === $type ) ) {
		$type = 'star';
	}
 ...
 *
 * @param int $post_id Post ID.
 * @return string      Empty string if user review is disabled.
 */
function wp_review_get_post_user_review_type( $post_id = null ) {
	...
	if ( ! in_array( $user_rating_setup, array( WP_REVIEW_REVIEW_VISITOR_ONLY, WP_REVIEW_REVIEW_COMMENT_ONLY, WP_REVIEW_REVIEW_ALLOW_BOTH ), true ) ) {
		return '';
	}
...
function wp_review_get_review_data( $post_id = null, $args = array() ) {
	...
	$data['author'] = ( empty( $author_field ) || ! $custom_author ) ? get_the_author() : $author_field;
...
function wp_review_post_list_column_content( $column_name, $post_id ) {
	if ( 'wp_review_rating' === $column_name ) {
		if ( ! wp_review_get_post_review_type( $post_id ) ) {
			echo '<span aria-hidden="true">—</span><span class="screen-reader-text">' . __( 'No Review', 'wp-review' ) . '</span>';
		} else {
			$total = get_post_meta( $post_id, 'wp_review_total', true );
			if ( $total ) {
				$args = array(
...
function wp_review_is_hidden_desc( $post_id ) {
	return get_post_meta( $post_id, 'wp_review_hide_desc', true );
}
...
function wp_review_get_review_items( $post_id = null ) {
	...
	if ( array() === $items ) { // Review items are edited and empty.
		/**
		 * Fallback review items.
		 *
		 * @since 3.4.10
		 */
		return apply_filters( 'wp_review_items_fallback', array() );
	}
	...
	if ( ! is_array( $items ) ) { // Review items have not edited or having errors.
		$items = array();
		$default_items = wp_review_get_default_criteria();
		...
		foreach ( $items as $index => $item ) {
			if ( empty( $item['id'] ) || is_numeric( $item['id'] ) ) {
				$items[ $index ]['id'] = wp_generate_password( 6 );
			}
		}
	}
	...
	/**
	 * Allows modifying review items after processing.
	 *
	 * @since 3.4.10
	 */
...
function wp_review_get_reviews_query( $type, $options ) {
	...
	if ( ! empty( $options['cat'] ) ) {
		$query_args['category__in'] = $options['cat'];
	}
	...
	if ( ! empty( $options['review_type'] ) ) {
		if ( in_array( 'star', $options['review_type'], true ) && wp_review_option( 'global_user_rating' ) ) {
			// Override user rating in old post is off and review type is none.
			$meta_query[] = array(
				'relation' => 'OR',
				array(
					'key'     => 'wp_review_override_global_user_rating',
					'value'   => 1,
					'compare' => 'NOT EXISTS',
				),
				array(
		...
		if ( ! wp_review_option( 'global_user_rating' ) ) {
			...
			// Override user rating in old post is off or review type is enabled.
			$meta_query[] = array(
				'relation' => 'OR',
				array(
					'key'     => 'wp_review_override_global_user_rating',
...
function wp_review_user_can_rate_features( $post_id, $type = 'visitor' ) {
	$meta_value = get_post_meta( $post_id, 'wp_review_user_can_rate_feature', true );
	return $meta_value ? $meta_value : wp_review_option( 'user_can_rate_feature' );
}
...
function wp_review_get_capabilities() {
	return array(
		...
		'wp_review_convert_schema'        => esc_html__( 'Convert schema', 'wp-review' ),
 ...
 * @since 3.3.7
 *
 * @param float  $total Rating value.
 * @param string $type  Rating type.
 * @return string
 */
function wp_review_get_rating_text( $total, $type ) {
	$rating_types = wp_review_get_rating_types();
	$total_text   = $total;
	if ( 'star' !== $type ) {
		$total_text = sprintf( $rating_types[ $type ]['value_text'], $total_text );
	}
	return $total_text;
}
 ...
 * @return string
 */
function wp_review_get_total_rating( array $review ) {
	return wp_review_rating(
		$review['total'],
		$review['post_id'],
		array(
			'review_total' => true,
 ...
 */
function wp_review_color_luminance( $hex, $percent ) {
	// Validate hex string.
...
/**
 * Prints the Google icon.
 *
 * @since 3.3.10
 *
 * @param bool $echo Show the output or return the string.
 * @return string|void
 */
function wp_review_google_icon( $echo = false ) {
	ob_start();
	?>
...
/**
 * Gets excluded post types.
 *
 * @since 3.3.11
 *
 * @return array
 */
function wp_review_get_excluded_post_types() {
	return apply_filters( 'wp_review_excluded_post_types', array( 'attachment' ) );
}


/**
 * Clears all transient which have name contains given strings.
 *
 * @since 3.4.2
 *
 * @param array $strings Contained strings.
 */
function wp_review_clear_transient_by_contained_string( array $strings ) {
	if ( empty( $strings ) ) {
		return;
	}
	...
	foreach ( $strings as $string ) {
		$where[] = "option_name LIKE '_transient_{$string}%'";
		$where[] = "option_name LIKE '_transient_timeout_{$string}%'";
	}
