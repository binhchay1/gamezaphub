...
/**
 * Class WP_Review_Yelp_Business_Reviews_Shortcode
 */
class WP_Review_Yelp_Business_Reviews_Shortcode {
	...
	public function render( $atts ) {
		...
		if ( ! $business ) {
			...
			if ( ! is_wp_error( $business ) ) {
				set_transient( $key, $business, wp_review_option( 'yelp_cache_expired_time', 7 ) * DAY_IN_SECONDS );
			}
		}
		if ( is_wp_error( $business ) ) {
			return $this->get_error_message( $response );
...
$shortcode = new WP_Review_Yelp_Business_Reviews_Shortcode();
$shortcode->init();
