...
class WP_Review_Facebook_Reviews_Shortcode {
	...
	protected function get_page_reviews( $page_id, $args = array() ) {
		...
		if ( ! is_wp_error( $response ) ) {
			set_transient( $key, $response, $this->get_cache_expired_time() );
		}
	...
	protected function get_page( $page_id ) {
		...
		$fb_api               = new WP_Review_Facebook_API();
		$api_version          = WP_REVIEW_GRAPH_API_VERSION;
		$url                  = "https://graph.facebook.com/v{$api_version}/{$page_id}?fields=name,overall_star_rating,rating_count";
		$args['access_token'] = $page_access_token;
	...
	/**
	 * Gets cache expired time.
	 *
	 * @return int
	 */
	protected function get_cache_expired_time() {
		return intval( wp_review_option( 'facebook_cache_expired_time', 7 ) );
	}
}
