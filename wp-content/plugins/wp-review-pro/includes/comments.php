...
function wp_review_comments_template( $theme_template ) {
	if ( is_singular() ) {
		...
		if ( in_array( $post->post_type, wp_review_get_excluded_post_types(), true ) || ! wp_review_get_post_review_type( get_queried_object_id() ) ) {
...
function wp_review_comment_add_title_to_text( $text, $comment = null ) {
	...
	if ( '' !== $rating ) {
		...
		if ( 'product' === get_post_type( $comment->comment_post_ID ) && 'theme' !== wp_review_option( 'comments_template', 'theme' ) ) {
			echo wp_review_verified_comment( $comment_id );
		}
...
function wp_review_preprocess_comment( $commentdata ) {
	...
	if ( in_array( $post_type, wp_review_get_excluded_post_types(), true ) || ! wp_review_get_post_review_type( $commentdata['comment_post_ID'] ) ) {
		return $commentdata; // Do nothing if this post type is excluded or review is disabled.
	}
...
function wp_review_comment_rating_input( $args = array() ) {
	...
	$rating_types         = wp_review_get_rating_types();
	$type                 = wp_review_get_post_user_review_type( $post->ID );
	$rating_type_template = $rating_types[ $type ]['input_template'];
	$post_id              = $post->ID;
	$value                = 0;
	$comment_rating       = true;
 ...
 *
 * WooCommece is adding comment meta upon comment submision, so we are just using their data.
 *
 * @param integer $comment_id Comment ID.
 * @return string
 */
function wp_review_verified_comment( $comment_id ) {
	if ( is_feed() ) {
		return '';
	}

	// We will use WooCommece option and keep our filter for backward compatibility.
	$show_verified_product = apply_filters( 'wpr_show_verified_label', 'yes' === get_option( 'woocommerce_review_rating_verification_label' ) );

	if ( $show_verified_product && function_exists( 'wc_review_is_from_verified_owner' ) && wc_review_is_from_verified_owner( $comment_id ) ) {
