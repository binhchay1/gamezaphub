...
class LCP_Controller extends Controller {
	...
	public function __construct() {
		...
		$this->register_action( 'wp_ajax_smush_handle_lcp_data', array( $this, 'handle_lcp_data' ) );
		$this->register_action( 'wp_ajax_nopriv_smush_handle_lcp_data', array( $this, 'handle_lcp_data' ) );
	...
	public function should_run() {
		return parent::should_run() && $this->settings->is_lcp_preload_enabled();
	}
	...
	public function handle_lcp_data() {
		// Verify nonce.
		...
		$new_data_hash         = $lcp_data->get_hash();
		if ( ! $lcp_data->is_lcp_element_image() ) {
			wp_send_json_error( array(
				'error_msg' => esc_html__( 'LCP element is not an image.', 'wp-smushit' ),
			) );
		}
		...
		if ( empty( $url ) || empty( $raw_data ) || empty( $data_store ) || ! is_a( $data_store, LCP_Data_Store::class ) ) {
			wp_send_json_error( array(
				'error_msg' => esc_html__( 'Error in processing LCP data, fields empty.', 'wp-smushit' ),
			) );
		}

		$version_changed = $previous_data_version === LCP_Helper::NO_DATA_VERSION || $current_data_version !== $previous_data_version;
		$hash_changed    = $previous_data_hash === LCP_Helper::NO_DATA_HASH || $new_data_hash !== $previous_data_hash;
		if ( ! $version_changed && ! $hash_changed ) {
			wp_send_json_error( array(
				'error_msg' => esc_html__( 'LCP data is already up to date', 'wp-smushit' ),
			) );
		}

		$data_store->save( $url, $is_mobile, $lcp_data );

		do_action( 'wp_smush_lcp_data_updated', $lcp_data, $data_store );
	...
	public function ajax_mark_all_lcp_data_as_dirty() {
		if ( ! check_ajax_referer( 'clear_all_lcp_data', 'nonce', false ) ) {
			wp_send_json_error( array(
