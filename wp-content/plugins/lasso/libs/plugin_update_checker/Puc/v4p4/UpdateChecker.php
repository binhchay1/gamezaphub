...
if ( !class_exists('Lasso_Puc_v4p4_UpdateChecker', false) ):

	abstract class Lasso_Puc_v4p4_UpdateChecker {
		...
		/**
		 * @var Lasso_Puc_v4p4_InstalledPackage
		 */
		protected $package;

		/**
		 * @var Lasso_Puc_v4p4_Scheduler
		 */
		public $scheduler;

		/**
		 * @var Lasso_Puc_v4p4_UpgraderStatus
		 */
		protected $upgraderStatus;

		/**
		 * @var Lasso_Puc_v4p4_StateStore
		 */
		...
		public function __construct($metadataUrl, $directoryName, $slug = null, $checkPeriod = 12, $optionName = '') {
			...
			$this->upgraderStatus = new Lasso_Puc_v4p4_UpgraderStatus();
			$this->updateState = new Lasso_Puc_v4p4_StateStore($this->optionName);
		 ...
		 *
		 * @return Lasso_Puc_v4p4_Update|null
		 */
		 ...
		 *
		 * @return Lasso_Puc_v4p4_StateStore
		 */
		...
		public function getUpdate() {
			...
			if ( isset($update) && isset( $update->version ) && $update->version ) {
		 ...
		 *
		 * @return Lasso_Puc_v4p4_Update An instance of Update, or NULL when no updates are available.
		 */
		 ...
		 * @param Lasso_Puc_v4p4_Update|null $update
		 */
		protected function fixSupportedWordpressVersion(Lasso_Puc_v4p4_Update $update = null) {
		 ...
		 * @return array [Lasso_Puc_v4p4_Metadata|null, array|WP_Error] A metadata instance and the value returned by wp_remote_get().
		 */
		protected function requestMetadata($metaClass, $filterRoot, $queryArgs = array()) {
			...
			$license_key = Lasso_License::get_license();
			...
			if((int)$wp_version >= 5) {
				...
				$info['is_classic_editor'] = $is_classic_editor;
			}
			...
			$options = array(
				'timeout'   => Lasso_Setting::TIME_OUT, // seconds
			...
			if ( !is_wp_error($status) ) {
				// return null if response code is 401 (unauthorized). It should not trigger error for unauthorized access.
				if(isset($result['response']['code']) && $result['response']['code'] === 401) {
					$metadata = null;
				} else {
			...
			return array($metadata, $result, $queryArgs);
		}
			...
			if ( !isset($result['response']['code']) ) {
				return new WP_Error(
					...
					'wp_remote_get() returned an unexpected result.'
                        ...
                        // It should not trigger error for unauthorized access (401).
			if ( $result['response']['code'] !== 200 && $result['response']['code'] !== 401) {
				return new WP_Error(
		...
		protected function createDebugBarExtension() {
			return new Lasso_Puc_v4p4_DebugBar_Extension($this);
		}

		/**
		 * Display additional configuration details in the Debug Bar panel.
		 *
		 * @param Lasso_Puc_v4p4_DebugBar_Panel $panel
		 */
